...
(function(){
     ...
     */
    DisplayObjectContainer.prototype.getObjectUnderPoint = function(x, y,returnAll)
    {
        ...
        for(var i = this.children.length - 1; i >= 0; i--)
        {
            ...
            {
                var obj = child.getObjectUnderPoint(x, y,returnAll);
                if(obj)
                {
                    if(returnAll) {if(obj.length > 0) result = result.concat(obj);}
                    else return obj;
                }else if(child.hitTestPoint(x, y) >= 0)
                {
